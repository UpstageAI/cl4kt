...
def model_train(fold, model, accelerator, opt, train_loader, valid_loader, test_loader, config, early_stop=True):
    train_losses = []
    avg_train_losses = []
    best_valid_auc = 0
    ...
    for i in range(1, num_epochs + 1):
        ...
        if valid_auc > best_valid_auc:
            ...
            path = os.path.join(os.path.join('saved_model', model_name, data_name), "params_*")
            ...
            torch.save({'epoch': i,
                        ...
                        os.path.join(os.path.join('saved_model', model_name, data_name), "params_{}".format(str(best_epoch)))
                      )
        if i - best_epoch > 10:
            break
        ...
        print(
            "Fold {}:\t Epoch {}\t\tTRAIN LOSS: {:.5f}\tVALID AUC: {:.5f}\tTEST AUC: {:.5f}"
            .format(fold, i, train_loss, valid_auc, test_auc)
        )
    checkpoint = torch.load(os.path.join(os.path.join('saved_model', model_name, data_name), "params_{}".format(str(best_epoch))))
    ...
    acc = accuracy_score(y_true=total_trues >= 0.5,
                        y_pred=total_preds >= 0.5)
    rmse = np.sqrt(mean_squared_error(
        y_true=total_trues, y_pred=total_preds))

    print(
        "Best Model\tTEST AUC: {:.5f}\tTEST ACC: {:5f}\tTEST RMSE: {:5f}"
        .format(auc, acc, rmse)
    )

    logs_df = logs_df.append(pd.DataFrame(
        {"EarlyStopEpoch": best_epoch, "auc": auc, "acc": acc, "rmse": rmse}, index=[0]), sort=False)

    log_out_path = os.path.join(log_path, data_name)
    os.makedirs(log_out_path, exist_ok=True)
    logs_df.to_csv(os.path.join(log_out_path, "{}_{}.csv".format(
        model_name, now)), index=False)
